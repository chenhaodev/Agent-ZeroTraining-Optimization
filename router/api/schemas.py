...
class RouteRequest(BaseModel):
    ...
    min_confidence: Optional[float] = Field(0.70, description="Minimum confidence for RAG (0.0-1.0)", ge=0.0, le=1.0)
...
class RouteResponse(BaseModel):
    ...
    use_rag: bool = Field(..., description="Whether to use RAG retrieval")
    rag_reason: str = Field(..., description="Explanation for RAG decision")
    rag_confidence: float = Field(..., description="Confidence in RAG decision (0.0-1.0)")
...
class PromptResponse(BaseModel):
    ...
    use_rag: bool = Field(..., description="Whether RAG should be used")
